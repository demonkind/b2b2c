...
ncDefine("nc.goGoodsPage", ["nc.eventManger"], function (eventManger) {
    ...
    GoGoodsPage.settings = {
        // 商品详情url前缀
        goodsInfoUrl: ncGlobal.webRoot + "activity/activityInfo?goodsId=",
        //规格值json
    ...
    Addcart.prototype = {
        ...
        bindEvents: function () {
            ...
            this.options.$buyNumAddBtn.click(function () {
                ...
                var b;
                	b=c+1;
                    if (c >= that.options.weight ){
                    	Nc.alertError("商品限购"+that.options.weight+"件");
                    	b=that.options.weight ;
	                }
                    if(c>=that.options.goodsStorage){
                		Nc.alertError("商品库存只有"+that.options.goodsStorage+"件");
                		b=that.options.goodsStorage ;
                	}
            ...
            this.options.$buyNumInput.on("keyup", function () {
                var $this = $(this),
                    ...
                    d = that.options.goodsStorage > that.options.weight ? that.options.weight:that.options.goodsStorage;
                    console.log(d);
                if (!Nc.isDigits($this.val())) {
                    Nc.getNum($this.val()) > d
                        ? $this.val(d)
                        : ($this.val(1));
            ...
            this.options.$buynowSubmitBtn.click(function () {
            	//验证是否购买数量是否大于限购数量
            	var parseBuyNum = parseInt(that.options.$buyNumInput.val());
            	if(parseBuyNum > parseInt(that.options.weight)){
	               	 Nc.alertError("商品限购"+that.options.weight+"件");
	               	 return false;
                }
            	//验证是否购买数量是否大于库存
            	if(parseBuyNum > parseInt(that.options.goodsStorage)){
	               	 Nc.alertError("商品库存"+that.options.goodsStorage+"件");
	               	 return false;
                }
            	//判断数量是否为0
            	if(parseBuyNum ==0){
	               	 Nc.alertError("请选择商品数量");
	               	 return false;
               }
                //验证是否登录
                ...
                $.getJSON(that.options.urlIsLogin, function (xhr) {
                    if (xhr.code == "200") {
                        if (xhr.data.status == true) {
                            that.options.cartId.val(that.options.goodsId + '|' + parseBuyNum);
                            $("#buynow_form").submit();
                        } else {
...
$(function () {
    ...
    // 活动介绍
    $('#tabActivityConsult').click(function () {
        $('.bd').css('display', 'none');
        $('#activityConsult').css('display', '');
        $('.hd').css('display', 'none');
    });
 // 联璧铃铛
    $('#tabLinktown').click(function () {
        $('.bd').css('display', 'none');
        $('#linktown').css('display', '');
        $('.hd').css('display', 'none');
    });
...
var countdown = function(){
	...
	$.each($tims,function(i,v){
		...
		var storage = $(this).attr("data-goodsStorage");//库存量
		var _this = $(this);
		var ss ; // 时间戳
		if(type == 0){ 
			if(starttime){
			 ...
			 t = setInterval(function(){
				...
				if(type ==0){
					...
					if(ss <= 0){
						...
						type = 1;
						if(atype==2){
							var href = $(_this).parents(".product_info").find(".favorite-btn a").attr("data-href","");
							$(_this).parents(".product_info").find(".favorite-btn a").attr("href",href).removeClass("disabled");
						}
					}
				}else{

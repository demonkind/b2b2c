...
function checkLogin(state){
	if(state == 0){
		location.href = WapSiteUrl+'/login';
		return false;
	}else {
		return true;
	}
...
function buildUrl(type, data) {
    switch (type) {
        case 'keyword':
            return WapSiteUrl + '/tmpl/product_list?keyword=' + encodeURIComponent(data);
        case 'special':
            return WapSiteUrl + '/special?special_id=' + data;
        case 'goods':
            return WapSiteUrl + '/tmpl/product_detail?goods_id=' + data;
        case 'url':
            return data;
    }
    return WapSiteUrl;
...
function getSearchName() {
	...
	if (keyword == '') {
	    if(getCookie('deft_key_value') == null) {
	        $.getJSON(WapSiteUrl + '/tmpl/product_detail', function(result) {
...
function getCartCount(key, expireHours) {
    ...
    if (getCookie('key') !== null && getCookie('cart_count') === null) {
        ...
        $.ajax({
            type:'post',
            url:WapSiteUrl+'/member/orders/list',
            data:{key:key},
            dataType:'json',
            async:false,
            success:function (result) {
...
//    });
    ...
    // TODO comment by yfb 由于现在的首页没有搜索框，暂不需要获取搜索框内默认的产品名称
//    getSearchName();
    // TODO  comment by yfb 首页无购物车的地方，所以暂时注释掉
//    getCartCount();
    getChatCount();// 导航右侧消息
    ...
    $(".fix-block-r,footer .gotop").click(function(){
    	 btn = $(this)[0];
     ...
     */
        areaSelected: function(options) {
            ...
            function _getAreaList() {
                $.ajax({//获取区域列表
                    type:'post',
                    url:WapSiteUrl+'/area/list',
                    data:{areaId:isNaN(ASID)?0:ASID},
                    dataType:'json',
                    async:false,
                    success:function(result){
                    	console.log(result.areaId);
                        
                        if (result.data.areaList.length == 0) {
                            _finish();
                            return false;
                        }
                        if (ASINIT) {
                        ...
                        if (typeof(myScrollArea) == 'undefined') {
                            if (typeof(IScroll) == 'undefined') {
                                $.ajax({
                                    url: WapSiteUrl+'/static/js/iscroll.js',
                                    dataType: "script",
                                    async: false
                                  });
                            }
...
function loadSeccode(){
    ...
    $.ajax({
        type:'get',
        url:WapSiteUrl+"/CaptchaHelper/createCaptcha",
        async : false,
        dataType: 'json',
        success:function(result){
    ...
    $("#codeimage").attr('src',WapSiteUrl+'/CaptchaHelper/createCaptcha?act=seccode&op=makecode&k='+$("#codekey").val()+'&t=' + Math.random());
}
/**
 * 收藏店铺
 */
...
function dropFavoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: WapSiteUrl + '/index.php?act=member_favorites_store&op=favorites_del',
        data: {key: key, store_id: store_id},
        dataType: 'json',
        async: false,
        success: function(result) {
...
function favoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: WapSiteUrl + '/favorite/goods/addFavoritesGoods',
        data:{key:key,goods_id:goods_id},
        dataType: 'json',
        success: function(result) {
            if (result.code == 200) {
	            $('#favoritesId').val(result.data);
                return_val = true;
            }
        }
    });
 ...
 * 取消收藏商品
 */
function dropFavoriteGoods(favoritesId){
    var key = getCookie('key');
    if (!key) { checkLogin(0); return; }
    if (favoritesId <= 0) {
    ...
    $.ajax({
        type: 'post',
        url: WapSiteUrl + '/favorites/goods/del',
        data: {key: key, favoritesId: favoritesId},
        dataType: 'json',
        success: function(result) {
            if (result.code == 200) {
                return_val = true;

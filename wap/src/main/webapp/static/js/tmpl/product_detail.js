                ...
                var goodsId=$("#goodsId").val();
                 if(!key){
                    ...
                    $.ajax({
                       ...
                       data:{key:key,goodsId:goodsId,buyNum:buynum},
                       type:"post",
                       success:function (result){
...
$('#buy-now').click(function(){
	 ...
	 $.ajax({
			type:'get',
			url: WapSiteUrl+"/login/status",	
			dataType:'json',
			success:function(result){
				var islogin = checkLogin(result.data.status);
				var goodsId=$('#goodsId').val();
				var saleNum=$('#popGoodsSaleNum').text();
				var buynum=$('#buynum').val();	
				if(parseInt(saleNum)<=0 || parseInt(saleNum)<parseInt(buynum)){
					console.log(parseInt(saleNum))				
					errorTipsShow("库存不足");
		            return;					
				}			
				...
				var buynum=$('#buynum').val();
				var directUrl = WapSiteUrl + '/product/goToBuy?goodsId='+goodsId +'&buynum='+buynum;
				if(islogin == false){
					addCookie('url',directUrl);
	...
	//如果库存为0，
	var saleNum=$("#popGoodsSaleNum").attr("popGoodsSaleNum");
	if(saleNum<=0){
		alert("库存不足，无法购买")
	}
	//限购件数为1，点击立即购买弹出提示框
	var maxWeight02=$("#buy-now").attr("maxWeight"); 
	var val=$("#buynum").val();
	if(val>maxWeight02){
		alert("超出了限购数量")		
	}		 
...
//点击收藏如果没登录跳转到登录页面，如果已经登录就显示已经收藏
...
$('#enshrine').click(function(){	
	 $.ajax({
			...
			success:function(result){
				...
				if(islogin == false){		
					 window.location.href= WapSiteUrl + '/login/popuplogin';
					 $(".round .pd-collect").removeClass(".favorate");
				 }else{
					 $(".round .pd-collect").addClass(".favorate");
				 }
			},			

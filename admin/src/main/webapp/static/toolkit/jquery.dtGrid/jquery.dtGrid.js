			...
			var dtGridObject = {
				...
				load : function(){
								...
								$.ajax({
									...
									success:function(datas){
										dtGridReflectionObj.hideProcessBar(function(){
											...
											dtGridReflectionObj.originalDatas = datas;
											dtGridReflectionObj.originalDatas = dtGridReflectionObj.originalDatas?dtGridReflectionObj.originalDatas:new Array();
											//处理基础数据集
											dtGridReflectionObj.baseDatas = dtGridReflectionObj.originalDatas.slice(0, dtGridReflectionObj.originalDatas.length);
											//处理分页属性
					...
					if(!dtGridReflectionObj.option.ajaxLoad||dtGridReflectionObj.option.loadAll){
						...
						//shopnc添加字段类型标识
						pager.ncColumnsType = dtGridReflectionObj.option.ncColumnsType;
						...
						if(dtGridReflectionObj.sortParameter&&dtGridReflectionObj.sortParameter.columnId){
							if(dtGridReflectionObj.sortParameter.sortType!=0){
								...
								advanceQuerySort.field = dtGridReflectionObj.sortParameter.columnId;
								advanceQuerySort.logic = dtGridReflectionObj.sortParameter.sortType;
								dtGridReflectionObj.advanceQueryParameter = new Object();
								dtGridReflectionObj.advanceQueryParameter.advanceQuerySorts = new Array();
								dtGridReflectionObj.advanceQueryParameter.advanceQuerySorts.push(advanceQuerySort);
							}
						}
				...
				constructGrid : function(){
					...
					if(dtGridReflectionObj.option.showHeader!=false){
						...
						for(var i=0; i<columns.length; i++){
							gridContent += '	<th columnNo="'+i+'" columnId="'+columns[i].id+'" class="dt-grid-header '+dtGridReflectionObj.getColumnClassForHide(columns[i])+' '+columns[i].headerClass+(columns[i].fastSort === false ? ' ' : ' can-sort')+'" style="'+columns[i].headerStyle+'">';
				...
				faseQuery : function(){
                    ...
                    //bycj [判断是否有 $.fn.bootstrapSwitch 这个插件 ]
                    var isBootstrapSwitch = typeof $.fn.bootstrapSwitch ==='function';
					...
					for(var j=0; j<dtGridReflectionObj.option.columns.length; j++){
						...
						if(column.fastQuery==true){
							...
							else if(column.type=='switch' && isBootstrapSwitch){
								// bycj [ 自定义一个开关类型 ]
								content += '<div class="col-sm-4"><input show-type-switch id="'+column.fastQueryType+'_'+column.id+
								'" type="checkbox" autocomplete="off" data-input-true="'+column.inputTrue+
								'"  data-input-false="'+column.inputFalse+'" data-input-name="'+column.inputName+
								'" data-off-text="'+column.inputFalseText+'" data-on-text="'+column.inputTrueText+'"/></div>';
							}
                            else{
								content += '<div class="col-sm-4">';
								if(column.type=='date'){
                    ...
                    //bycj[ 自定义的switch 生成 ]
                    isBootstrapSwitch && ( $el.find("input[show-type-switch]").bootstrapSwitch({}) );
				...
				doFaseQuery : function(form){
					...
					//console.log(" bycj [ 添加默认搜索条件 ] ");
					if (typeof  dtGridReflectionObj.option.ncDefaultFastQueryParameters != "undefined"){
						var a = dtGridReflectionObj.option.ncDefaultFastQueryParameters;
						for(defaultQuery in a ){
							dtGridReflectionObj.fastQueryParameters[defaultQuery] = a[defaultQuery];
							dtGridReflectionObj.fastQueryParameters[defaultQuery+'_format'] = null;
						}
					}
					dtGridReflectionObj.load();
				},

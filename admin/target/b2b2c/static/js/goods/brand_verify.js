...
window.Parsley.addValidator('checkapplystate', {
    validateString : function(value) {
        console.log(value.length)
        if ($("#applyStateSwitchChange").attr("checked") != "checked") {
            if (value.length > 0) {
                return true;
            }else{
                return false;
...
grid.fastQueryParameters['ne_applyState'] = 1;
...
var brand = function () {
    ...
    function _bindEvent() {
        ...
        $("#editModal").on("show.bs.modal", function (e) {
            ...
            $.post(ncGlobal.adminRoot + 'brand/verify.json',
                {brandId: gridData.brandId},
                function(data){
                    if (data.code == 200) {
                        var brand = data.data.brandAndLabel;
                        var brandApply = data.data.brandApply;

                        $('#editFormBrandId').val(brand.brandId);
                        $('#editFormBrandName').val(brand.brandName);
                        $('#editFormBrandEnglish').val(brand.brandEnglish);
                        $('#editFormBrandImage').val(brand.brandImage);
                        $('#editFormBrandImageImg').attr('src', brand.brandImageSrc);
                        ...
                        $('#editFormBrandSort').val(brand.brandSort);
                        ...
                        if (brand.brandLabelIds != null) {
                            $.each(brand.brandLabelIds.split(','), function (i, e) {
                                $("#editFormBrandLabelsAll").find('div[data-id="' + e + '"]').click();
                            })
                        }

                        $('#registerNumber').val(brandApply.registerNumber);
                        $('#image1').attr('src', brandApply.image1Src);
                        $('#image2').attr('src', brandApply.image2Src);
                        $('#owner').val(brandApply.owner);

                        $('#applyStateSwitchChange').bootstrapSwitch('state', true)
                            .on('switchChange.bootstrapSwitch', function(event, state) {
                                if (state == true) {
                                    $('#stateRemark').parents('.form-group:first').hide();
                                } else {
                                    $('#stateRemark').parents('.form-group:first').show();
                                }
                            });

                        $('#image1,#image2').jqthumb({
                            width: 110,
                            height: 150,
                            after: function (imgObj) {
